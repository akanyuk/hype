	...
	<footer class="topic-footer">
		...
		<ul class="topic-info clearfix">
			...
			<li class="topic-info-views"><i class="icon-views"></i> {$oTopic->getCountRead()}</li>
		...
		<div id="vote_area_topic_{$oTopic->getId()}" class="vote">
			...
			{if !$bVoteInfoShow}
				<div class="vote-item vote-zero" title="{$aLang.topic_vote_count}: {$oTopic->getCountVote()}" onclick="return ls.vote.vote({$oTopic->getId()},this,0,'topic');" style="padding: 0 10px;">Воздержаться</div>
			{/if}
