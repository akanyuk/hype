jQuery(document).ready(function($){
	...
	// Переносим тулбар "следующий комментарий на место"
	if ($('#next-comment-container').length > 0) {
		var html = $('#next-comment-container').html();
		$('#next-comment-toolbar').html(html);
		$('#next-comment-container').remove();	
	}
	else {
		$('#next-comment-toolbar').remove();
	...
	// Unread comments / unread news toolbar
	if ($(document).find('#unread-stream').length) {
		$('.icon-stream-lg').addClass('unread');
	}
	
	if ($(document).find('#unread-news').length) {
		$('.icon-news-lg').addClass('unread');
	}
	
		 
	// Userbar
	var isUserbarHidden = true;
	var iUserbarWidth = 220;
	
	ls.tools.showuserbar = function(force) {
		if (!isUserbarHidden && force !== true) return;
		isUserbarHidden = false;
		
		//$('iframe').hide();	// Костыль для андроида (?)
		$('#wrapper').addClass('hidden');
		$('#userbar-trigger').addClass('active');
		$('#userbar').css('left', 0);
	};


	ls.tools.hideuserbar = function(force) {
		if (isUserbarHidden && force !== true) return;
		isUserbarHidden = true;

		
		//$('iframe').show();
		$('#wrapper').removeClass('hidden');
		$('#userbar-trigger').removeClass('active');
		$('#userbar').css('left', -iUserbarWidth);
	};


	$('#userbar-trigger').click(function(){
		isUserbarHidden ? ls.tools.showuserbar() : ls.tools.hideuserbar();			
	});
		...
		$(document).swipe({
			swipeStatus: function(event, phase, direction, distance, duration, fingerCount) {
				...
				case 'move':
					...
					var processUserbar = !isVerticalLock && (direction == 'right' && isUserbarHidden || direction == 'left' && !isUserbarHidden) ? true : false;
					if (!processUserbar) return;
					
					if (direction == 'right') {
						var pos = distance > iUserbarWidth ? 0 : distance - iUserbarWidth;
						$('#userbar').css('left', pos);
					}

					if (direction == 'left') {
						var pos = distance > iUserbarWidth ? -iUserbarWidth : -distance;
						$('#userbar').css('left', pos);
					}
				...
				case 'cancel':
					...
					if (direction == 'left') {
						ls.tools.hideuserbar(true);
						return;
					}
					
					var pos = parseInt($('#userbar').css('left'));
					pos > -(iUserbarWidth / 2) ? ls.tools.showuserbar(true) : ls.tools.hideuserbar(true);						
				}
			},
			//excludedElements: $.fn.swipe.defaults.excludedElements + ", #slider, li, i, .nav-foldable-trigger, #header",
			excludedElements: $.fn.swipe.defaults.excludedElements + ", .nav-foldable-trigger",
	...
	$(window).resize(function () {
		$('#userbar-inner').css('min-height', $(window).height() + 100);
		ls.tools.bindswipe();
	}).trigger('resize');
...
*/
	...
	ls.comments.scrollToComment = function(idComment) {
		$.scrollTo('#comment_id_'+idComment, 1000, { offset: -48 });
		...
		// $('#comment_id_' + idComment).removeClass(this.options.classes.comment_new);
	...
	// Скролл
	// Отключено после перехода jQuery.ScrollTo 1.4.2 -> 2.1.2
	// by nyuk
	// $(window)._scrollable();
